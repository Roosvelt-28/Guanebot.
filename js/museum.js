...
// Sombra de contacto donde la pared se encuentra con el piso: un degradado
// simple de oscuro (junto al muro) a transparente (hacia el centro de la
// sala). No depende de la cultura — es el mismo truco de "AO falso" que
// ancla visualmente cualquier pared al piso bajo luz plana.
function crearTexturaSombraContacto(izquierda) {
  const w = 64, h = 8;
  const c = document.createElement('canvas'); c.width = w; c.height = h;
  const x = c.getContext('2d');
  const grad = izquierda ? x.createLinearGradient(0, 0, w, 0) : x.createLinearGradient(w, 0, 0, 0);
...
function construirMuseo(piezas) {
  ...
  // Filtrado anisotrópico: sin esto, el piso y el techo (vistos casi de
  // canto, en un pasillo largo) se ven borrosos y con "ruido" tembloroso
  // al moverse — el problema típico de texturas tileadas sin este filtro.
  const aniso = ren.capabilities.getMaxAnisotropy();
  [texSuelo, texTecho, texPared, texGreca, texSimbolos].forEach(t => { t.anisotropy = aniso; });
  ...
  // Sombra de contacto pared-piso (una por lado; el degradado es uniforme a
  // lo largo, no hace falta repetirlo)
  const texSombraL = crearTexturaSombraContacto(true);
  const texSombraR = crearTexturaSombraContacto(false);
  const mSombraL = new THREE.MeshBasicMaterial({ map: texSombraL, transparent: true, depthWrite: false });
  const mSombraR = new THREE.MeshBasicMaterial({ map: texSombraR, transparent: true, depthWrite: false });
  ...
  for (let s = 0; s < SALAS.length; s++) {
    ...
    // Sombra de contacto donde cada pared toca el piso
    plano(0.9, SL, mSombraL, -RW / 2 + 0.45, 0.013, oz - SL / 2, -Math.PI / 2, 0);
    plano(0.9, SL, mSombraR,  RW / 2 - 0.45, 0.013, oz - SL / 2, -Math.PI / 2, 0);
...
function ponerLuces() {
  // Antes esto estaba tan fuerte (2.4 / 1.4 de ambient + 0.7-0.9 direccional
  // + puntual por sala) que aplanaba cualquier textura — todo se veía del
  // mismo tono sin contraste. Bajarlo dejará ver el grano/las vetas/las
  // sombras de las texturas nuevas y se sentirá mucho más real.
  scene.add(new THREE.AmbientLight(0xF0E6CE, esMovil ? 1.1 : 0.62));
  const d = new THREE.DirectionalLight(0xFFF3DC, esMovil ? 0.45 : 0.55);
  d.position.set(5, 10, 5); scene.add(d);
  if (!esMovil) {
    // Una sola PointLight por sala (antes 2), más cálida y con caída más
    // marcada — crea charcos de luz en vez de iluminar todo por igual.
    for (let s = 0; s < SALAS.length; s++) {
      const oz = -s * SL - SL / 2;
      const pt = new THREE.PointLight(0xFFDDA0, 0.85, SL * 1.1, 1.8);
